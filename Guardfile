...
guard 'sprockets', :destination => 'js', :asset_paths => ['sjs'], :root_file   => 'sjs/head.js', :minify => false do
  watch(%r{^sjs/.*\.(js|coffee)})
end

guard 'sprockets', :destination => 'js', :asset_paths => ['sjs'], :root_file   => 'sjs/libs.js', :minify => false do
  watch(%r{^sjs/.*\.(js|coffee)})
end
